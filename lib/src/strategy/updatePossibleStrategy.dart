...
bool updatePossibleStrategy(Grid grid) {
  var explanation = 'Set Possible';
  var updated = false;
  grid.grid.forEach((r) => r.forEach((c) {
        if (allNonetsUpdateCell(grid, c, explanation)) updated = true;
      }));
  return updated;
}

/// Update [cell] possible values using its box, row and column
bool allNonetsUpdateCell(Grid grid, Cell cell, String explanation) {
  var updated = false;
  ...
  if (nonetUpdateCell(grid, cell, cells, location)) updated = true;
...
bool nonetUpdateCell(
    Grid grid, Cell cell, List<Cell> cells, String explanation) {
  var updated = false;
