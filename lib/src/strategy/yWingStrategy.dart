...
bool yWingStrategy(Grid grid) {
  var explanation = 'Y-Wing';
  return yWing(grid, explanation);
}

bool yWing(Grid grid, String explanation) {
  var updated = false;
        ...
        for (var major1 in majors.keys) {
          ...
          if (cell1.possibleCount == 2 && cell2.possibleCount == 2) {
            ...
            if (value1 != value2) {
              ...
              var box1 = cell1.box;
              var box2 = cell2.box;
              if (yWingCheckBox(grid, valuePossibleBoxes, value1, box1, cell1,
                  value2, box2, axis, explanation)) updated = true;
              if (updated && grid.singleStep) return true;
              if (yWingCheckBox(grid, valuePossibleBoxes, value2, box2, cell2,
    ...
    if (otherCell.possibleCount == 2 &&
      ...
      cells.where((c) => grid.axisEqual(axis, c, cell)).forEach((c) {
        if (c.clearPossible(otherValue)) {
          updated = true;
          grid.cellUpdated(cell, location, "remove value $otherValue from $c");
        }
      });
